...
class PostalAddress(BaseModel):
    ...
    postal_code: Optional[str] = Field(None, alias="postalCode", description="Country specific postal code")
...
class Site(BaseModel):
    ...
    postal_address: Optional[PostalAddress] = Field(None, alias="postalAddress", description="Postal address")
    type: SiteType = Field(..., description="Type of site")
    address_classification: Optional[SiteIpAddressClassification] = Field(
        None, 
        alias="addressClassification", 
        description="IP address classification"
    )
    architecture: Optional[List[LayerSet]] = Field(None, description="Logical network topology/architecture")
    site_market: Optional[str] = Field(None, alias="siteMarket", description="Name of the Site Market this site belongs to")
    peeringdb_site_mapping: Optional[str] = Field(
        None, 
        alias="peeringdbSiteMapping", 
...
class CreateSiteMarketRequest(BaseModel):
    ...
    site_market: SiteMarket = Field(..., alias="siteMarket", description="Site market configuration to create")


class CreateSiteMarketResponse(BaseModel):
    """Response from creating a site market."""
    
    site_market: SiteMarket = Field(..., alias="siteMarket", description="Created site market configuration")
...
class ListSiteMarketsResponse(BaseModel):
    ...
    site_markets: List[SiteMarket] = Field(
        ...
        alias="siteMarkets",
        description="List of configured site markets"
    )
    invalid_count: Optional[int] = Field(
        None, 
        alias="invalidCount",
...
class GetSiteMarketResponse(BaseModel):
    ...
    site_market: SiteMarket = Field(..., alias="siteMarket", description="Site market configuration")


class UpdateSiteMarketRequest(BaseModel):
    """Request to update an existing site market."""
    
    site_market: SiteMarket = Field(..., alias="siteMarket", description="Updated site market configuration")
